...
def main():
    ...
    res = subprocess.Popen(get_train_cmd(Model, env, Model_path))
    res.wait()

    prog_print(f"Training random env for {Model} in {env}")
    res = subprocess.Popen(get_train_cmd(Model, get_random_name(env), Model_path))
    res.wait()

    prog_print(f"Eval DD for {Model} in {env}")
    res = subprocess.Popen(get_eval_cmd(Model, env, env, Model_path, eval_folder))
    res.wait()

    prog_print(f"Eval DR for {Model} in {env}")
    res = subprocess.Popen(get_eval_cmd(Model, env, get_random_name(env), Model_path, eval_folder))
    res.wait()

    prog_print(f"Eval RR for {Model} in {env}")
    res = subprocess.Popen(
        get_eval_cmd(Model, get_random_name(env), get_random_name(env), Model_path, eval_folder)
    )
    res.wait()

    prog_print(f"Eval DE for {Model} in {env}")
    res = subprocess.Popen(get_eval_cmd(Model, env, get_extreme_name(env), Model_path, eval_folder))
    res.wait()

    prog_print(f"Eval RE for {Model} in {env}")
    res = subprocess.Popen(
        get_eval_cmd(Model, get_random_name(env), get_extreme_name(env), Model_path, eval_folder)
    )
    res.wait()
